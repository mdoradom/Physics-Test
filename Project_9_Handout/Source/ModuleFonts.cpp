...
int ModuleFonts::Load(const char* texture_path, const char* characters, uint rows)
{
	...
	App->textures->GetTextureSize(font.texture, font.totalLength, font.char_h);

	font.columns = strlen(characters);

	font.char_w = font.totalLength / strlen(characters);
...
void ModuleFonts::BlitText(int x, int y, int font_id, const char* text) const
{
	...
	for(uint i = 0; i < len; i++) {
		...
		int c;
		for (c = 0; c < fonts[font_id].totalLength; c++) {
			if (fonts[font_id].table[c] == text[i]) {
				break;
			}
		}

		uint col = c % fonts[font_id].rows;
		uint row = c / fonts[font_id].rows;

		spriteRect.x = col * fonts->char_w;
		spriteRect.y = row * fonts->char_h;

		App->render->Blit(font->texture, x + i * font->char_w, y, &spriteRect, 1.0f, false);
	}
}
