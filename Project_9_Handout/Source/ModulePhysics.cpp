...
bool ModulePhysics::Start()
{
	...
	rightSquare.y = 11.3f; // [m]
	rightSquare.w = 2.0f; // [m]
	rightSquare.h = 3.3f; // [m]

	// Create downSquare
	downSquare = Square();
	downSquare.x = 20.0f; // [m]
	downSquare.y = 11.2f; // [m]
	downSquare.w = 4.0f; // [m]
	downSquare.h = 0.2f; // [m]
...
Update_Status ModulePhysics::PreUpdate()
{
	...
	for (auto& ball : balls)
	{
		...
		if (is_colliding_with_square(ball, leftSquare)) {
			// TP ball to ground surface
		...
		else if (is_colliding_with_square(ball, topSquare))
		{
			// TP ball to ground surface
...
Update_Status ModulePhysics::PostUpdate()
{
	...
	App->render->DrawQuad(ground.pixels(), color_r, color_g, color_b, 255);

	// Draw water
	color_r = 0; color_g = 0; color_b = 255;
	App->render->DrawQuad(water.pixels(), color_r, color_g, color_b, 255);
	...
	for (auto& ball : balls)
	{
		...
		color_r = 255; color_g = 0; color_b = 150;
